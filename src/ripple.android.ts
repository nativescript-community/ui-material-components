...
export class Ripple extends RippleBase {
    ...
    setRippleDrawable(view: android.view.View) {
        if (android.os.Build.VERSION.SDK_INT >= 23) {
            ...
            view.setBackground(this.createCompatRippleDrawable(this.getRippleColor(this.style['rippleColor'])));
            //   view.setForeground(
    ...
    [rippleColorProperty.setNative](color: string) {
        if (android.os.Build.VERSION.SDK_INT >= 23) {
            (this.nativeViewProtected.getBackground() as any).setColor(android.content.res.ColorStateList.valueOf(new Color(color).android));
        } else {
